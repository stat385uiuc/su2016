---
title: "Lecture 10: Piping and Webscrapping"
...
```{r smore_bunny_foo_foo, out.width = "220px", fig.retina = NULL, fig.align='center', cache = TRUE, echo = FALSE}
...
`x` may *not* the first function parameter. e.g.
...
- Yes, the period or `.` is how to indicate a redirection.
...
- In English: **This is not a pipe**.
    - Follows from [René Magritte's The Treachery of Images](https://en.wikipedia.org/wiki/The_Treachery_of_Images)
...
- Any questions on the **Piping Operator**?
- Up next, we're looking at **Web Scrapping**.
...
Let's focus a bit on scrapping the [stat.illinois.edu](http://www.stat.illinois.edu) website

Specifically, we're going to visit the [PhD Student Directory](http://www.stat.illinois.edu/people/grad.shtml)

#  Simple `rvest` example - Finding the Selectors

- Go to the [PhD Student Directory](http://www.stat.illinois.edu/people/grad.shtml)
...
To generalize it, we'll aim to drop the `nth-child(1)` selector
...
2. Why is a `#` next to `content` but not `ul`? 
